<template>
  <div class="header-wrapper">
    <div class="logo-container">
      <a href="/"
        ><img
          src="@/assets/aresources/logo/logo_brown_bear.png"
          alt="Logo"
          class="logo"
      /></a>
    </div>
    <div class="user-container">
      ...
      <div v-else>
        ...
        <Tooltip content="登出">
          <a href="/login-resource"
            ><Button type="text" @click="logout">退出</Button></a
          >
        </Tooltip>
      </div>
    </div>
    ...
    >
      ...
      >
        ...
        <div class="user-info">
          <img
            :src="
              user.scUserAvatars ||
              'http://sfrsrdckw.hn-bkt.clouddn.com/usertximg/logo_duck_c.png'
            "
            alt="用户头像"
            class="user-avatar"
          />
        ...
        <Divider />
        <p style="margin-top: 5px; zoom: 0.82">
          <strong>个人留言：</strong>{{ user.userBio }}
        </p>
      </Card>
      <Menu mode="vertical" active-name="1-1" @on-select="handleSelect">
        ...
        <Submenu name="4" title="我的群组">
          <template #title>
            <Icon type="md-contacts"></Icon> 我的群组
          </template>

          <MenuItem name="4-1">
            <Icon type="ios-beaker" /> 我的消息通知
          </MenuItem>
          <MenuItem name="4-2"> 
            <Icon type="md-list-box" /> 好友列表 
          </MenuItem>
          <MenuItem name="4-3">
            <Icon type="ios-chatboxes" /> 我的群组
          </MenuItem>
        </Submenu>

        <Submenu name="5" title="用户设置">
          <template #title><Icon type="md-people" /> 账号管理</template>
          <MenuItem name="5-1" style="margin-top: 8px">
          ...
          <MenuItem name="5-4" @click="confirmDeactivation">
            <Icon type="md-alert" />
            注销账号
          </MenuItem>
        </Submenu>
        <MenuItem name="6" v-if="admin">
    ...
    <Modal
      v-model="deactivationConfirmVisible"
      title="确认注销账号"
      @on-ok="showPasswordModal"
    >
  ...
  methods: {
    ...
    handleSelect(name) {
      if (name === "1-1") {
        ...
        this.$router.push({ name: "my-group", query: { index: true } });
        this.$Message.info("此功能还未完善，等待后续开发");
      } else if (name === "4-2") {
        this.$router.push({ name: "my-group", query: { friends: true } });
        this.$Message.info("此功能还未完善，等待后续开发");
      } else if (name === "4-3") {
        this.$router.push({ name: "my-group", query: { groups: true } });
        this.$Message.info("此功能还未完善，等待后续开发");
      } else if (name === "5-1") {
